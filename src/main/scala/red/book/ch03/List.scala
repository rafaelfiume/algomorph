...
object List { // `List` companion object. Contains functions for creating and working with lists.
  ...
  def myappendWithSomeTrace[A](l: List[A], r: List[A]): List[A] = foldRight(l, r)((a, b) => { println(s"a: $a; b: $b"); Cons(a, b) })
  ...
  def filter[A](list: List[A])(f: A => Boolean): List[A] = foldRight(list, Nil:List[A]) { (a, b) => if (f(a)) Cons(a, b) else b }

  def flatMap[A](list: List[A])(f: A => List[A]): List[A] = foldRight(list, Nil:List[A])((a, b) => myappend(f(a),b))

  def filterWithFlatMap[A](list: List[A])(f: A => Boolean): List[A] = flatMap(list)(a => if (f(a)) List(a) else Nil)

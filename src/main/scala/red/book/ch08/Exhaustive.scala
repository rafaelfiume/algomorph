...
object Exhaustive {
  ...
  def forAll[A](as: Gen[A])(f: A => Boolean)(implicit stream: StreamGenerator[A], testInspector: TestInspector[A]): Prop = Prop { (n, rng) =>
...
object StreamGenerator {
  ...
  implicit def aStream[A]: StreamGenerator[A] = (n, rng) => gen => {
  ...
  implicit def byteStream: StreamGenerator[Byte] = { (_,_) => _ =>
    Stream.from(Byte.MinValue).map(_.toByte).zip(Stream.from(0)).take(abs(Byte.MinValue) + Byte.MaxValue + 1)
  }
  implicit def booleanStream: StreamGenerator[Boolean] = { (_,_) => _ =>
    Stream(true, false).zip(Stream.from(0)).take(2)
  }
...
object TestInspector {
  ...
  implicit val byteResultAnalyser = new ProvableTestInspector[Byte]
  implicit val booleanResultAnalyser = new ProvableTestInspector[Boolean]
  implicit def listResultAnalyser[A] = new UnfalsifiedTestInspector[List[A]]

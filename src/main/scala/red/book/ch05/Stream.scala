...
sealed trait Stream[+A]:
  ...
  def takeWhile_w_fold(p: A => Boolean): Stream[A] =
    foldRight(empty[A])((x, acc) => if p(x) then cons(x, acc) else empty) // my original solution used `else acc` instead
  ...
  def filter(p: A => Boolean): Stream[A] = foldRight(empty[A])((x, acc) => if p(x) then cons(x, acc) else acc)
  ...
  def startsWith[B](other: Stream[B]): Boolean = zipAll(other).takeWhile(_._2.nonEmpty).forAll { case (a, b) =>
    a == b
  }
...
object Stream:
  ...
  def unfoldViaFoldRight[A, S](z: S)(f: S => Option[(A, S)]): Stream[A] = f(z) match
    case None         => empty
    case Some((a, s)) => cons(a, empty).foldRight(empty[A])((x, _) => cons(x, unfoldViaFoldRight(s)(f)))

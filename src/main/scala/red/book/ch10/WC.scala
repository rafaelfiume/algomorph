  ...
  val wcMonoid = new Monoid[WC]:
    override def op(a1: WC, a2: WC): WC =
      val r = (a1, a2) match
        ...
        case (Part(l1, n1, r1), Part(l2, n2, r2)) => Part(l1, n1 + n2 + (if (r1 ++ l2).contains(" ") then 1 else 0), r2)
      r

    override def zero: WC = Stub("")

  def countWords(s: String): Int =
    def unstash(s: String) = if s.trim.isEmpty then 0 else 1
    def toWC(c: Char) = if c == ' ' then Part(" ", 0, " ") else Stub(c.toString)

...
trait Applicative[F[_]] extends Functor[F]:
  ...
  def product[G[_]](G: Applicative[G]): Applicative[({ type f[a] = (F[a], G[a]) })#f] =
...
object ApplicativeInstances:
  ...
  def validatedApplicative[E]: Applicative[({ type f[x] = Validated[E, x] })#f] =
    new Applicative[({ type f[x] = Validated[E, x] })#f]:

...
trait Traverse[F[_]] extends Functor[F] with Foldable[F] {
  ...
  def compose[G[_]](implicit G: Traverse[G]): Traverse[({type f[x] = F[G[x]]})#f] = {
    ...
    new Traverse[({type f[x] = F[G[x]]})#f] {
      override def traverse[M[_] : Applicative, A, B](fa: F[G[A]])(f: A => M[B]): M[F[G[B]]] = F.traverse(fa)(ga => G.traverse(ga)(f))
    }
  }
...
object Traverses {
  ...
  def composeM[F[_], G[_]](F: Monad[F], G: Monad[G], T: Traverse[G]) = new Monad[({type f[x] = F[G[x]]})#f] {
    ...
    override def flatMap[A, B](fga: F[G[A]])(f: A => F[G[B]]): F[G[B]] = F.flatMap(fga)(ga => F.map(T.traverse(ga)(f)(F))(ggb => G.join(ggb)))
  }
...
object Test extends App {
  ...
  // map
  println {
    listTraverse.map(List(1,2,3,4))(_.toString) // shouldBe List("1","2","3","4")
  }
  println {
    listTraverse.map(List(1,2,3,4))(_ - 1) // shouldBe List(0,1,2,3)
  }
  println {
    optionTraverse.map(Some(5))(_ * 2) // shouldBe Some(10)
  }
  println {
    optionTraverse.map(empty[Int])(_ * 2) // shouldBe None
  }

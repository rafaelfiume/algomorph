...
object Strings {
  ...
  def isOneWay(s1: String, s2: String): Boolean = {
    def oneEditReplace(s1: String, s2: String): Boolean = {
      (s1 zip s2).foldLeft(0) { case (count, (a, b)) => if (a == b) count else count + 1 } == 1
    }

    def oneEditInsert(one: String, another: String): Boolean = {
      val indexes = one.foldLeft((0, 0)) { case ((indexA, indexB), _) =>
        if (one.charAt(indexA) != another.charAt(indexB)) {
          (indexA, indexB + 1)
        } else {
          (indexA + 1, indexB + 1)
        }
  ...
  def compress(s: String): String = {
    ...
    r.chars.foldLeft(new StringBuilder(r.length)) { case (st, c -> count) =>
  ...
  def isRotation(original: String, s2: String): Boolean = {
    ...
    if (length != s2.length || length == 0) return false

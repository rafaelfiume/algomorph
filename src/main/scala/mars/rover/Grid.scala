...
object Grid:
  ...
  type Weight = Int
  case class Edge(node: Node, w: Weight):
    override def toString: String = s"[n=$node;w=$w]"
  ...
  def unweighted(rows: Int, columns: Int, obstacles: List[Node] = Nil): UnweightedGrid = for
    ...
    left = (r, if c - 1 < 0 then columns - 1 else c - 1)
    up = (if r - 1 < 0 then rows - 1 else r - 1, c)
    down = (if r + 1 >= rows then 0 else r + 1, c)
    right = (r, if c + 1 >= columns then 0 else c + 1)

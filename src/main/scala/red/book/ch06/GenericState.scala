...
object RngState {
  ...
  def nonNegativeLessThan(n: Int): Rand[Int] = for {
      i <- nonNegativeInt
      mod = i % n
      v <- if (i + (n - 1) - mod >= 0) unit(mod) else nonNegativeLessThan(n)
    } yield v
...
object State {
  ...
  def sequence[S, A](ls: List[State[S, A]]): State[S, List[A]] = ls.foldRight(unit[S, List[A]](empty)){ (x, acc) => x.map2(acc)(_ :: _)}

  def traverse[S, A, B](ls: List[A])(op: A => State[S, B]): State[S, List[B]] = sequence { ls map op } // TODO Very lazy way of implementing traverse... Improve this if up to this task
...
object Machine {
  ...
  def unlock(m: Machine): Machine = if (m.locked && m.candies > 0) m.copy(unlocked = true, coins = m.coins + 1) else m
  def turn(m: Machine): Machine = if (m.unlocked) m.copy(unlocked = false, candies = m.candies - 1) else m

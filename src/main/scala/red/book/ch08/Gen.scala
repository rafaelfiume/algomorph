...
object Prop {
  ...
  def check[A](f: => Boolean): Prop = Prop { (_, _, _) => if (f) Proved else Falsified("()", 0) }
...
object Gen {
  ...
  def choose(start: Int, stopExclusive: Int): Gen[Int] = Gen(RngState.nonNegativeInt map (n => start + n % (stopExclusive-start)))
  ...
  def streamOfN[A](n: Int, g: Gen[A]): Gen[Stream[A]] =  listOfN(n, g) map { list => Stream(list: _*) }
  ...
  def option[A](gen: Gen[A]): Gen[Option[A]] = choose(1, 21) flatMap { i =>
    if (i == 10) unit(None) else gen.map(Some(_))
  }

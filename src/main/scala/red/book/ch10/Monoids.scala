...
object Monoids:
  ...
  def foldMapV[A, B](as: IndexedSeq[A], m: Monoid[B])(f: A => B): B =
    if as.isEmpty then return m.zero // does this make sense? it does.
    if as.size == 1 then return f(as(0))
  ...
  val monOrd = new Monoid[Track]:
    override def op(a1: Track, a2: Track): Track = (a1, a2) match
      ...
      case (False(), _)                       => False()
      case (_, False())                       => False()
      case (TNumber(n1), TNumber(n2))         => if n1 <= n2 then Interval(n1, n2) else False()
      case (Interval(n1, n2), TNumber(n3))    => if n2 <= n3 then Interval(n1, n3) else False()
      case (TNumber(n1), Interval(n2, n3))    => if n1 <= n2 then Interval(n1, n3) else False()
  ...
  def orderedFF(as: IndexedSeq[Int]): Boolean =
    ...
    val t = as.tail
    t.tail
      .foldLeft(TrackInterval(h, t.head, true)) {

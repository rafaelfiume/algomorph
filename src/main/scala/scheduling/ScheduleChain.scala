...
object ScheduleChain:
  ...
  def make[R, C <: ChainContinuity](schedules: Seq[Schedule[R]])(sort: Seq[Schedule[R]] => Seq[Schedule[R]])(
    validate: Rule[R]
  ): Either[List[Error], ScheduleChain[R, C]] =
    val sorted = sort(schedules)
    validate(sorted).map(_ => new ScheduleChain[R, C](sorted) {})

  def makeContinuous[R](schedules: Seq[Schedule[R]]): Either[List[Error], ScheduleChain[R, Continuous]] =
    make(schedules)(_.sortBy(_.startMillis))(continuous)

  def makeContinuousAssumeSorted[R](schedules: Seq[Schedule[R]]): Either[List[Error], ScheduleChain[R, Continuous]] =
    make(schedules)(identity)(continuous)

  def makeWithGaps[R](schedules: Seq[Schedule[R]]): Either[List[Error], ScheduleChain[R, WithGaps]] =
    make(schedules)(_.sortBy(_.startMillis))(noConflicts)

  def makeWithGapsAssumeSorted[R](schedules: Seq[Schedule[R]]): Either[List[Error], ScheduleChain[R, WithGaps]] =

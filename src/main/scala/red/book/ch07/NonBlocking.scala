...
object Nonblocking {
  ...
  object Par {
    ...
    def choiceN[A](p: Par[Int])(ps: List[Par[A]]): Par[A] = es => (cb: A => Unit) => {
      p(es) { b =>
        eval(es) { ps(b)(es)(cb) }
      }
    }
    ...
    def choiceMap[K,V](p: Par[K])(ps: Map[K,Par[V]]): Par[V] = es => (cb: V => Unit) => {
      p(es) { key =>  ps(key)(es)(cb) }
    }

    // see `Nonblocking.scala` answers file. This function is usually called something else!
    def chooser[A,B](p: Par[A])(f: A => Par[B]): Par[B] = flatMap(p)(f) // yeah, this is a flatMap
    ...
    def choiceViaChooser[A](p: Par[Boolean])(f: Par[A], t: Par[A]): Par[A] = flatMap(p) { b => if (b) f else t }

    def choiceNChooser[A](p: Par[Int])(choices: List[Par[A]]): Par[A] = flatMap(p) {i => choices(i)}
    ...
    def flatMapViaJoin[A,B](p: Par[A])(f: A => Par[B]): Par[B] = join { map(p)(a => f(a)) }

...
object Path:
  ...
  def shortestPath(start: Node, end: Node, grid: UnweightedGrid): List[Node] =
    ...
    @tailrec
    def bdf(visited: List[Node], queue: Queue[Node], parents: Parents): Parents =
      if queue.isEmpty then return parents
      if queue.head == end then return parents
    ...
    val parents = bdf(List(start), Queue(neighboursFromStart*), startNeighboursParent)

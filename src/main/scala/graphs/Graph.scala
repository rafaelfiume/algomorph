...
import graphs.traversal.Dfs.EdgeClassifier
...
class Graph[V <: Vertex] private (
  ...
  // In particular, it will override all the edges of `vertex` that have been added with the fn `add(Edge)`.
  /*
...
// TODO Optimise `add` ops, perhaps by replacing Map[V, List[V]] by Map[V, Vector[V]] and List[V] by LinkedHashSet[V]?/s

...
sealed trait Option[+A] {
  ...
  def filter(p: A => Boolean): Option[A] = flatMap (v => if (p(v)) Some(v) else None)
...
object Option {
  ...
  def traverse[A, B](list: List[A])(f: A => Option[B]): Option[List[B]] = list.foldRight(some(empty[B]))((x, xs) => map2(f(x), xs)(_ :: _))
  ...
  def sequence3[A](list: List[Option[A]]): Option[List[A]] = list.foldRight(some(empty[A]):Option[List[A]]) { (o, list) => (o, list) match {
    case (None, _) => None
    case (_, None) => None
